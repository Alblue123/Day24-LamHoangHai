...
def validate_anonymized_data(filepath: str) -> dict:
    ...
    # Check 1: Không còn CCCD gốc dạng số thuần túy (giả sử PII detector hoạt động tốt)
    # Ở đây ta check nếu có dòng nào cccd chỉ toàn số và độ dài 12 (có thể là leak)
    # Tuy nhiên trong lab này, ta chỉ check sơ bộ
    leaks = df[df['cccd'].astype(str).str.match(r'^\d{12}$')]
    if len(leaks) > 0:
        results["success"] = False
        results["failed_checks"].append("Potential PII leak in 'cccd' column")

    # Check 2: Không có null values trong các cột quan trọng
    for col in ["patient_id", "ho_ten", "benh"]:
        if df[col].isnull().any():
            results["success"] = False
            results["failed_checks"].append(f"Null values found in '{col}'")

    # Check 3: Số rows phải khớp (không mất mát dữ liệu)
    # (Giả sử 200 là số mặc định)
    if len(df) < 200:
        results["success"] = False
        results["failed_checks"].append("Data loss detected (row count < 200)")
